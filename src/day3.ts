...
function getPoints(lines: string[]): Point[] {
    let current = new Point(0, 0);
    const result = [];
    for (const l of lines) {
        const d = directions[l[0]];
        for (let step = 0; step < parseInt(l.substr(1)); step++) {
...
/*const points1 = getPoints(line1).map(p => p.toString());
const points2 = getPoints(line2).map(p => p.toString());
const pointsSet1 = new Set(points1);
const pointsSet2 = new Set(points2);

const intersections: Set<string> = intersection(pointsSet1, pointsSet2);
// console.log('intersections: ', intersections);

const minDistance = Math.min(...[...intersections].map(p => Point.fromString(p)).map(p => Math.abs(p.x) + Math.abs(p.y)));
const minSteps = Math.min(...[...intersections].map(p => 2 + points1.findIndex(p1 => p === p1) + points2.findIndex(p2 => p === p2)));

console.log('minDistance: ', minDistance); // a
console.log('minSteps: ', minSteps); // b*/
