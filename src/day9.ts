...
if (!equals(test1.output, [109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99])) {
    console.error('failed test1, result: ', test1);
}
let test2 = run({...program, output: [], state: [1102, 34915192, 34915192, 7, 4, 7, 99, 0]});
if (!test2.output || `${test2.output[0]}`.length !== 16) {
    console.error('failed test2, result: ', test2);
}
let test3 = run({...program, output: [], state: [104, 1125899906842624, 99]});
if (!test3.output || test3.output[0] !== 1125899906842624) {
    console.error('failed test3, result: ', test3);
}
let test4 = run({...program, output: [], state: [3, 9, 8, 9, 10, 9, 4, 9, 99, -1, 8], remainingInputs: [8]});
if (!equals(test4.output, [1])) {
    console.error('failed test4, result: ', test4);
}
let test5 = run({...program, output: [], state: [3, 9, 8, 9, 10, 9, 4, 9, 99, -1, 8], remainingInputs: [9]});
if (!equals(test5.output, [0])) {
    console.error('failed test5, result: ', test5);
}
let test6 = run({...program, output: [], state: [3, 3, 1107, -1, 8, 3, 4, 3, 99], remainingInputs: [7]});
if (!equals(test6.output, [1])) {
    console.error('failed test6, result: ', test6);
}
let test7 = run({...program, output: [], state: [3, 3, 1107, -1, 8, 3, 4, 3, 99], remainingInputs: [8]});
if (!equals(test7.output, [0])) {
    console.error('failed test7, result: ', test7);
}
let test8 = run({
    ...program,
    output: [],
...
if (!equals(test8.output, [1])) {
    console.error('failed test8, result: ', test8);
}
let test9 = run({
    ...program,
    output: [],
...
if (!equals(test9.output, [0])) {
    console.error('failed test9, result: ', test9);
}*/
